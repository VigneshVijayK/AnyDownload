...
function proxyCmd(base: string): string {
  if (!HTTP_PROXY) return base;
  return base.replace(/^curl /, `curl -x "${HTTP_PROXY}" `);
}

let lastRequestTime = 0;
async function rateLimitedFetch(url: string, cmd: string): Promise<string> {
  const now = Date.now();
...
async function fetchWithCurl(url: string): Promise<string> {
  ...
  return rateLimitedFetch(url, cmd);
}
...
async function fetchProfileMedia(username: string): Promise<{
  items: MediaItem[];
  profile: any;
  nodes: RawNode[];
}> {
  const cacheKey = `profile:${username}`;
  const cached = cacheGet<{ items: MediaItem[]; profile: any; nodes: RawNode[] }>(cacheKey);
  ...
  try {
    ...
    throw new Error(
      "Instagram returned an invalid response. They may be blocking automated requests. Try again later."
    );
  }
...
async function resolvePostOwner(shortcode: string, mediaType: string = "post"): Promise<string> {
  ...
  // Use the embed/captioned page which contains profile links with usernames
  const path = pathForType(mediaType);
  const url = `https://www.instagram.com/${path}/${shortcode}/embed/captioned/`;

  // Use Safari UA so Instagram serves the compact embed page (not the full JS app)
  const cmd = `curl -sS --max-time 15 "${url}" \
  ...
  const { stdout } = await execAsync(proxyCmd(cmd),
  ...
  throw new Error(
    "Could not resolve the post owner from this URL. Try searching by username instead."
  );
}
  ...
  try {
    ...
    switch (type) {
      ...
      case "story": {
        ...
        return NextResponse.json({
          items: singleItems,
          profile: { ...result.profile, relatedProfiles: [] },
        });
      }
